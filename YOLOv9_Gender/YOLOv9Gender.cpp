...
bool YOLOv9Gender::Initialize(const char* model_path, int model_width, int model_height)
{
...
std::vector<Object> YOLOv9Gender::Detect(cv::Mat image)
{
    timeval start, stop;
    char buffer[80];
    preProcess(image);
...
void YOLOv9Gender::preProcess(cv::Mat& image)
{
    blobFromImage(image);
}

std::vector<Object> YOLOv9Gender::postProcess(float scaleX, float scaleY)
{
    _proposals.clear();
...
void YOLOv9Gender::generate_proposals(float scaleX, float scaleY, float prob_threshold)
{
...
std::vector<int> YOLOv9Gender::nms(float nms_threshold)
{
